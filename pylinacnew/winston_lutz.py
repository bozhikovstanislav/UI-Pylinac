...
class WinstonLutz:
    ...
    def plot_images(self, axis: str=ALL, show: bool=True):
        ...
        # set titles
        fig.suptitle("{} images".format(axis), fontsize=8,x=0.1, y=0.95)
        #plt.subplots_adjust(top=2.5,bottom=1.8)
        plt.tight_layout()
        if show:
            plt.show()
    ...
    def publish_pdf(self, filename: str, notes: Union[str, List[str]]=None, open_file: bool=False, metadata: dict=None):
        ...
        for ax in (GANTRY, COLLIMATOR, COUCH, COMBO):
            if self._contains_axis_images(ax):
                ...
                data = io.BytesIO()
                self.save_images(data, axis=ax, figsize=(18, 18))
                canvas.add_image(data, location=(3, 8), dimensions=(18, 18))
